...
import CustomerProducts from "../../components/app/customers/CustomerProducts";
...
const UserDetails = () => {
  ...
  const getRentals = async (query = "") => {
    try {
      ...
      const { data } = await axios.get(
        `/booking/rentalHistory?id=${id}&type=${type}&role=user&search=${query}${
          initialDate
  ...
  useEffect(() => {
    getRentals();
  }, [update, isApplied, currentPage, activeTab, type]);
  useEffect(() => {
    getStatus();
  }, []);
      ...
      <div className="w-full   flex flex-col  py-4  justify-start items-start gap-6">
        ...
        <div className="w-full overflow-x-auto lg:overflow-x-hidden flex flex-col justify-start items-start gap-6">
          <div className="w-full flex lg:flex-row flex-col lg:justify-between lg:items-center">
            ...
            <div className="w-[270px] flex  px-[2px] py-[2px] border border-gray-300 bg-white h-[49px] rounded-[8px]">
              <button
                onClick={() => setType("own")}
                className={`min-w-[90px] px-4 py-2 capitalize text-sm font-normal leading-[17.58px] ${
                  type === "own"
          ...
          {loading ? (
            [1, 2, 3, 5, 6, 7, 8]?.map((item) => (
              <div className="bg-white w-full rounded-xl" key={item}>
